...
home = ["dataclass"]
# Только внутри приложения: dataclass в загрузчике или в наблюдаемости — это
# три поля рядом, а не DTO.
area = "application"
...
# У отказа две половины: фраза — человеку, её можно переписать; код — тому, кто
# на него ветвится, и его переписать нельзя.
[refusals]
zones = ["modules"]
carries = "refusal"
...
# Блок на фреймворк, и в блоке — только то, чего этот сервис требует от входа.
# Что такое вход — имена методов, декоратор это или вызов, как пишется первый
# аргумент — знает правило: так решили авторы фреймворков, и таблица об этом
# была бы пересказом чужой документации, стареющим вместе с ней.
...
# Что мутировать — таблица самого mutmut; здесь только то, что нужно гейту:
# где лежит запись и под каким профилем тянут примеры тесты свойств, пока их
# мутируют, — свежий пример на каждый прогон убивает мутанта один раз и
# упускает в следующий, и запись начинает двигаться сама.
[mutation]
...
| Полнота декларации на входе | правило `edge-declarations`: FastAPI и FastStream дают слова, но ничего не требуют; Schemathesis проверяет соответствие коду, а не полноту схемы |
...
| Выжившие мутанты | `py-checks mutation` поверх `mutmut`: область — mutmut'а, запись — проекта, три прогона — библиотеки |
...
### `mutation` — судят тесты, а не код

Выживший мутант — строка, которую поменяли, а все тесты прошли.
`py-checks mutation` гоняет `mutmut` и отказывает пушу, который оставляет в
модуле больше выживших, чем числится за ним в записи:

```bash
py-checks mutation diff     # модули, которые тронула ветка, — то, что гоняет пуш
py-checks mutation full     # всё, что мутирует mutmut, модуль за модулем
py-checks mutation record   # полный прогон, записанный как новая запись
```
...
Библиотека mutmut не импортирует — зовёт процессом, как `schema-drift` зовёт
alembic, — но читает то, что он печатает, и формат вывода и есть договор между
ними. Extra закрепляет мажор, в котором этот формат держится: разбор,
промахнувшийся мимо каждой строки `survived`, насчитал бы ноль и пропустил бы
всё.

Место ему — на пуше, а не в CI: отчёт нужен тому, кто сейчас запушит тесты,
которые он судит, а раннер CI — самая слабая машина, на какую только можно
потратить минуты мутаций.
...
| Ключ | По умолчанию | |
|---|---|---|
| `command` | `["mutmut"]` | как звать mutmut |
| `baseline` | `"mutation-baseline.json"` | куда `record` пишет выживших по модулям |
| `against` | `["origin/develop", "develop"]` | с чем сравнивать ветку, когда пуш этого не сказал |
| `children` | решает mutmut | сколько мутантов разом; `--children` перекрывает |
| `env` | `{}` | переменные для прогона |

Что мутировать, в этой таблице нет. Это `source_paths` и `do_not_mutate`
самого mutmut, и читаются они оттуда же, откуда их читает mutmut: из
`[tool.mutmut]`, если он есть в `pyproject.toml`, иначе из `[mutmut]` в
`setup.cfg`. Область, сказанная дважды, — это две области, которые разойдутся.
Проект, не сказавший её нигде, получает отказ: гейт, не знающий, что
мутируется, судил бы пустоту и пропускал бы всё.

**Почему по модулям.** Запись — карта, а не число, и `full` судит её модуль за
модулем так же, как `diff`. Сумма стоит на месте, когда в одном модуле
выживших прибавилось, а в другом столько же убыло, — и новая дыра проходит под
прикрытием чужой работы.

**Почему выживших гоняют ещё раз.** Кэш mutmut привязан к мутированному
исходнику, поэтому мутант, которого убил тест, написанный сегодня, числится
живым: в его файле ничего не поменялось. `diff` называет модули, которые
судит, `full` — выживших, в которых верит кэш, и именно имя заставляет mutmut
прогнать их заново. Считаются два вердикта, `survived` и `no tests`; второй
сильнее — модуль без единого теста иначе давал бы ноль. `not checked` — отказ:
когда падает сбор тестов, mutmut всё равно выходит с единицей, и по коду
выхода такой прогон не отличить от прогона с выжившими.

**Вместо `tools/check_mutation_gate.py`.** Его несли три сервиса, в трёх
версиях — на 514, 600 и 700 строк, — и исправление для `not checked` дошло
до двух из них.
...
| `area` | `class-placement` | часть дерева, где притязание вообще действует |
...
**Притязание достаёт ровно туда, куда сказано в `area`.** Соглашение про
`dto` написано про слой приложения, а dataclass в загрузчике или в
наблюдаемости — способ сложить три поля рядом, а не предмет разговора. `area`
говорит, где `home` и `suffix` этого блока вообще о чём-то говорят; без неё —
везде.
...
Зона складывается из директорий. `domain` берёт `domain/` и всё, что под ним,
но не `application/exceptions/domain.py`: модуль, которому досталось имя слоя,
этим слоем не становится. Имя самого файла отзывается только на `*`, поэтому
`domain/*` по-прежнему берёт `domain/exceptions.py`. Так же устроен любой ключ
`zones` и `sealed-imports`.
...
### 5.7 api — что объявляет вход

#### `edge-declarations` — вход не сказал, как он себя ведёт
...
Блок на фреймворк, и называет он одно: чего требует этот сервис. Всё
остальное про вход решили те, кто писал фреймворк: `router.post` — декоратор,
и путь у него пишется словом `path=`; `broker.subscriber` не реже обычный
вызов, и топик у него стоит первым и без имени. Таблица, повторяющая это
здесь, была бы пересказом чужой документации, устаревающим в день её правки.

Этим же правило держится честным насчёт имён. Маршрут ищется только
декоратором: `get`, `post` и `delete` — те самые имена, которыми зовут клиента
HTTP, и правило, читающее любой `.post(...)` в дереве, нашло бы маршрут в
первом же адаптере к соседнему сервису. Подписка ищется где угодно: `subscriber`
здесь больше ничем не зовут.

Блок, названный фреймворком, о котором правило не знает, отвергается по имени,
и известные перечисляются рядом.

**Зачем.** Декоратор входа — это контракт. Кто читает сгенерированную
...
Первый аргумент — единственное в декораторе, чей смысл зависит от позиции.
Путь пишется словом `path=`, и переданный позиционно отвергается; топик
пишется первым и без имени, и подписка, назвавшая его словом, отвергается
тоже. И то и другое — соглашения самих фреймворков, поэтому в таблице их нет.
...
Подписка — второй вход в том же слое, и у неё нет ни статуса, ни модели
ответа. Объявляет она другое: что будет с записью — какой топик, под чьей
группой, как читаются байты, когда двигается офсет и откуда начинает новая
группа. За каждым из этих решений стоит умолчание FastStream, и каждое
умолчание — решение, принятое проектом иначе: `AckPolicy.REJECT_ON_ERROR`
роняет запись, которую брокер только что доставил заново,
`auto_offset_reset="latest"` пропускает то отставание, ради которого новая
группа и заводится, а автоматический декодер превращает байты в словарь
раньше, чем inbox увидел, что пришло.
...
**Пометка.** `# api-ok: edge-declarations: <причина>`.
...
### 5.10 errors — чем отказывает модуль

#### `refusals` — отказ, на который нечем ответить
...
**Зачем.** У отказа две половины. Фраза — человеку: её читают в логе, её
переписывают, и зависеть от её слов нельзя ничему. Код — то, на что ветвится
вызывающий: «денег не хватило» — это экран, «корзина изменилась» — это
пересчёт. Поэтому фраза вольна меняться, а код нет, и держится это ровно до
тех пор, пока каждое «нет», которое говорит модуль, несёт его с собой.

Встроенное исключение не несёт ни того ни другого. `ValueError`, пересёкший
сценарий, — это отказ, о котором снаружи известно только то, что он случился,
и периметр честно превращает его в 500: больше ему сделать не из чего.
`allow` называет встроенные, которые отказом не считаются:
`NotImplementedError` — это не «нет», а метод, которого ещё нет, и
единственное исключение, которое читатель не спутает с ответом.

`carries` — имя поля, которым свою ошибку наделяют кодом
(`raise BasketChanged(refusal=Refusal.BASKET_CHANGED)`). Не назвать его —
значит судить только встроенные, и для сервиса, у которого своего словаря
кодов ещё нет, это всё правило целиком.

`internal` называет свои ошибки, которые никому снаружи не «нет»: они говорят,
что код написан неверно, что порт нарушил обещание или что запись противоречит
сохранённой. Ветвиться на них некому, а ответ на них — повтор или страница
дежурному.

**Вместо ruff.** `TRY002` срабатывает на `raise Exception(...)` и больше ни на
чём — `ValueError` ровно так же лишён кода и проходит мимо. `TRY003` говорит о
длине сообщения, а не о том, можно ли с ним что-то сделать.

**Пометка.** `# error-ok: refusals: <причина>`.
