...
bot и betting судят все четыре имени, beauty и trading — только `commit` и
...
`# type-ok: annotation-shapes: заголовки, а не поля`. В bot и betting так
...
| Расхождение моделей и миграций | команда `python-checks drift` поверх `alembic check`: статикой это не видно, нужна база |
...
| Полнота `.env.example` | генератор `pydantic-settings-export` плюс `config-fields` с `alias` |
...
Это правило судит не файл исходника, а проект, и потому объявлено иначе — в
группе `python_checks.project_checks`. Зовут его один раз за прогон, вместе с
остальными: `python-checks run`.

## `.env.example` не проверяется, а собирается

Файл выводится из классов настроек, а не поддерживается рядом с ними:
[pydantic-settings-export](https://pypi.org/project/pydantic-settings-export/)
собирает его из моделей, и расходиться становится нечему. Цена — генератор
импортирует настройки, то есть запускает код приложения.
...
половинами — это то, ради чего у alembic есть `check`. Поэтому команда одна и
ей нужна база:
...
Базу она приносит свою — пустой файл во временной директории, накатанный от
...
- `env.py` проекта обязан читать адрес базы из переменной (`variable`). Где он
  берёт его из своих настроек, `drift` подсунуть базу не может — там шаг CI
