...
[tool.python-checks.function-length]
max-lines = 50
```

Считаются строки как написаны, вместе с пустыми и комментариями: держать в
голове читателю приходится их все. У функции считается только тело —
декораторы и сигнатура, разложенная по строкам, описывают функцию, а не
работу, которую она делает.

`PLR0915` из ruff это не заменяет, хотя выглядит похоже: он считает инструкции.
На четырёх сервисах самая длинная функция — 91 строка и две инструкции, и с
предела в 50 инструкций не падает ни одна функция из 3364 файлов. Включённый
вместо правила про строки, он его молча выключает.

Пометка снимается с любой строки сигнатуры: `# signature-ok: function-length:
одна настройка, читается целиком`. Так живут десять функций в четырёх сервисах
— настройка логирования цепочкой процессоров, строка таблицы, выписанная по
колонке, список подписок по одной на источник. Разрезанные пополам ради числа,
они кладут порядок в два места.
