...
[[tool.python-checks.argument-count.subjects]]
suffix = "UseCase"
max-arguments = 3
```
...
Вход сценария несёт то, что пришло снаружи, и вход длиннее трёх полей — это
вещь с именем: команда, запрос, DTO. Ключ здесь класс, а не директория: рядом
со сценарием в той же директории живут хелперы, у которых свои резоны на
четыре аргумента. Конструктор не в счёт — через него сценарий получает
зависимости, а это проводка, не вход.

`PLR0913` из ruff это не заменяет: у него нет ни привязки к классу, ни
исключения для конструктора. С `max-args = 3` он даёт 29, 142, 96 и 108
срабатываний по сервисам, а внутри одних только `use_cases` — 49, и все до
единого приходятся на `__init__`.
...
| Число аргументов входа | правило `argument-count`: `PLR0913` не знает ни классов, ни исключения для конструктора |
