...
Тот же `py-checks sync` — файл собирается вместе с контрактами. Перечисляются
секции, а не один корневой класс: корень собирает их фабриками, и его
собственные поля — это секции, а не переменные. В файл едут имя переменной,
значение по умолчанию, первый абзац докстринга класса и `description` поля,
если оно у него есть, — так объяснение живёт рядом с полем, а не в файле,
который его переживёт.
