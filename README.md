...
- repo: https://github.com/Armontex/py-checks
  ...
  hooks:
    ...
    - id: py-checks-sync     # контракты импортов собраны заново
```

Хук один, а не по хуку на правило: что включено, решает конфиг проекта. Одно
правило зовут `args: [--select, <код>]`.
...
| `py-checks run --select <код>` | только названные правила, несмотря на `ignore` |
...
| `py-checks sync [--check]` | собрать контракты импортов под раскладку проекта |
