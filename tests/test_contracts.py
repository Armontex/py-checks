...
def test_the_project_may_write_the_header_itself(tmp_path: Path) -> None:
    layout(tmp_path, "src/app/domain", "src/app/shared")
    own: dict[str, TomlTable] = {
        "contracts": {
            **SERVICE["contracts"],
    ...
    text = render(root=tmp_path, config=Config(checks=own)) or ""

    assert text.startswith("# Generated by `py-checks sync`. Do not edit.\n\n")
    assert "Контракты импортов" not in text
